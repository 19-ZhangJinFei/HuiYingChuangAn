<template>
    <svg t="1741142686482" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
        p-id="26093" width="200" height="200" @mouseover="hover = true" @mouseleave="hover = false"
        @click="changecolor">
        <path
            ...
            :fill="hover ? '#f9f221' : (isActive ? '#f9f221' : color)" p-id="26094">
        </path>
    </svg>
</template>
...
const isActive = ref(false); // 新增一个状态来跟踪是否点击激活

const changecolor = () => {
    isActive.value = !isActive.value; // 切换isActive状态
    // 如果点击后激活，且当前颜色不是#f9f221（即原始颜色），则更改为#f9f221
    // 否则，改回原始颜色#218ef9
    if (isActive.value && color.value !== '#f9f221') {
        color.value = '#f9f221';
    } else if (!isActive.value && color.value === '#f9f221') {
        color.value = props.color;
    }
    // 注意：这里我们不需要在悬浮时改变color的值，因为hover已经通过:fill属性直接处理了
};
</script>
